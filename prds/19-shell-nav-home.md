...
- **Key files**:
  ...
  - `src/components/user-menu.tsx` — **Admin + Feedback links live here** (admin-gated), under an "Administration" label.

- **Post-merge correction (2026-07-01, Dan):** the build originally folded **Admin** into the Advisory nav group. Reverted: Advisory is the *financial-stewardship* zone (trust documents + finances) and none of it is built, so **nothing should appear under Advisory yet**. **Admin and Feedback moved to the user menu** (admin-gated); Admin/Feedback removed from `nav-config.ts`'s Advisory group, which now holds only its two `soon` items → the Advisory top-nav trigger renders for no one. `UserMenu` regained an `isAdmin` prop (threaded from `site-header.tsx`). The Advisory **homepage door** still shows its two coming-soon rows (Documents & AI, Finances) as a roadmap signal. **Coming-soon items also now render at the bottom of the top-nav dropdowns + mobile accordion** as muted "Soon" rows (via `soonItemsForViewer`), mirroring the doors — so the Family dropdown lists Messaging as a muted Soon row. **A group whose only items are `soon` (Advisory) also renders a nav trigger** (`navGroupsForViewer` counts soon items), so the Advisory dropdown previews Documents & AI + Finances as muted Soon rows. Admin/Feedback remain in the user menu — they are not part of the Advisory group. `isGroupActive` counts soon items too, so a `/coming-soon/*` stub highlights its own mode.

- **Decisions made during build**:
  - ~~**Admin folds into the Advisory group**~~ — **reversed post-merge** (see correction above); Admin + Feedback live in the user menu.
