...
- **Slice 3 — Timeline**: ✅ **shipped** _(build session 2026-06-30; branch `claude/timeline-slice3`)_. Detailed spec in [10-family-timeline.md](10-family-timeline.md); this is the parent record.
  ...
  - **Follow-ups**: (1) apply the migration to prod + live-verify (as slices 1–2 did); (2) themed views (`/family/timeline/themes/[tag]`) — `events.tags` exists but no UI yet; (3) date ranges (`event_date_end`) deferred — single point/year per event for now; (4) slice 4 (Stories) can hang off events/people/albums next.
