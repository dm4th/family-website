...
- **Drag-and-drop onto the intake cards — ADDED (PR #38, merged 2026-07-31).** Dan asked to drag a picture or PDF of a bill straight in rather than using the picker. The drop target is the card, not the page, so the intent decision stays where the card-owned file inputs already keep it. Depth-counted highlight, window-level preventDefault (a missed drop can't navigate the tab away from an unsaved review), first file only, drops inert while an extraction runs, hints gated to fine-pointer devices. Dropped files go through the existing `handleFile`, so validation and resize are unchanged.
- **Blocked-microphone silence — FIXED (PR #37, merged 2026-07-31).** Dan hit a dead-looking mic button on desktop; root cause was a browser-level "Block" on the site's mic permission, which kills the Web Speech session in ~100ms with no prompt, and the hook's silent-error design gave zero feedback. `not-allowed` / `service-not-allowed` now render a line under the button naming the way out (site settings, or the keyboard mic), cleared on the next attempt. Transient errors stay silent as designed. The preflight-permission-query variant (surface the message before any click) was considered and deferred.
