...
| [15 — Guest Access](15-guest-access.md) | 🔍 in-review (code complete; needs live RLS verification + prod push) | — _(build isolated)_ | Property-scoped guest role: the first real member/guest RLS differentiation + `property_guests` join + middleware/page gating. **Negative tests are acceptance criteria.** Build carefully and not in parallel with 13/16 (nav) or other RLS-touching work. |
