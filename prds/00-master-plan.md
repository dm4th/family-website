...
| [19 — Shell Nav & Home](19-shell-nav-home.md) | 🚧 in review — built + PR'd (2026-07-01) | 18, 20 | Group the crowded top nav into **mode dropdowns** (Family/Operations/Advisory, accent-tinted, via shadcn `NavigationMenu`) and restructure the homepage into **three calm doors** instead of a link wall. Shared `nav-config.ts` drives nav + homepage so they can't drift; Admin folded into the Advisory group (out of the user menu); coming-soon pages render as muted "Soon" rows inside their door. Guest-nav gating preserved. Design-only; no new tables/routes/deps. `tsc`+`eslint`+`build` green; interactive verification (keyboard/guest/iPad) pending. |
