...
| [32 — Smart Intake](32-smart-intake.md) | 🚧 slice 1 in review ([PR #32](https://github.com/dm4th/family-website/pull/32), 2026-07-30); slices 2–3 🟢 ready | most | **The typing-relief unlock for Dad.** Photograph a bill/note → server-side Claude vision extracts fields → the **existing** edit form opens **pre-filled** → member reviews + Saves. **AI never writes; only pre-fills a form a human confirms** (stays inside the PRD-27 gated write path + `recordRevision`). Three sequenced slices, each behind Dan's review: **1 property details from a bill** (vendor `property_contacts` + address), **2 handwritten notes → guidelines/how-to/contact notes**, **3 calendar events from due dates**. New dep: `ANTHROPIC_API_KEY` (server-scope only) + one AI SDK. |
