...
| [36 — Property Key Info](36-property-key-info.md) | ✅ **shipped** (PR #43 merged `3c744b7`; migration applied to prod 2026-08-01; reviewer live-walked contacts-by-kind → Emergencies panel with fixed 911, ground panel, Service Directory table, 3 revisions with kind diffs, guard trigger confirmed wifi-free at the DB. **Wi-Fi panel verified live 2026-08-01** with owner-entered test data: panel order correct, Copy Password announces, and the inline QR's module bitmap is byte-identical to a locally generated code for the stored values. Real values arrive via the PRD-37 extractor; phone camera-join check happens then) | 35 | **Structure-first half of the "have an opinion on unstructured data" arc.** One additive migration: `properties.wifi_network`/`wifi_password` (member-editable, deliberately not privileged) + `property_contacts.kind` (`emergency`/`on_the_ground`/`service`, default backfills today's panel). Right column becomes Emergencies (fixed 911 + emergency contacts, always first) → Wi-Fi (password + Copy button + **server-rendered Wi-Fi QR** a phone camera joins from) → Contacts on the Ground → Amenities; full-width Service Directory below the grid. Guests see key info by design. Also fixes the edit-form hint that currently *invites* Wi-Fi and passwords into prose; account-credential vault explicitly deferred to 07/08. Intake untouched (that's 37's seam). |
| [37 — Paste Anything Ingestion](37-paste-anything-ingestion.md) | ✅ **shipped** (PR #45 merged `fc5907f` 2026-08-01; reviewer ran **the real Dad-doc walk on prod** same day: one press pulled the live 4,382-char blob in, advisory named all four credential entries with none leaking into any proposal, 26 digit-exact contacts proposed / 23 saved in one press as 23 gated writes with revisions, ambiguous Wi-Fi line honestly low-confidence and dismissed via Not This One, blob trimmed in-box to 724 chars of sectioned markdown + guidelines to the one real rule, pasted source opened verbatim then deleted — bucket + table back at zero. Eval: 0 leaked / 0 fabricated over 16×2 runs, 100% planted-credential catch) | 35 | **The filler half.** Third intake door "Paste Text": paste a Google Doc/email/manual → new `paste` intent (text path shared with dictation: fenced, store-`.txt`-before-model, PRD 33 retention for free) proposes contacts **with kinds** (bulk checklist review, N individually-gated saves), a Wi-Fi card, reminders, and tidied prose via append semantics (existing-blob cleanup happens in the same box). **Credential catch is the load-bearing safety piece**: login/password material is parse-excluded from every proposal, surfaced only as a redacted advisory — Dad's real doc has three plaintext utility logins on a guest-visible page today. Eval with a **scrubbed** Dad-doc fixture (real credentials never enter the repo); 100% planted-credential catch required. First real run = Dad's actual doc, then trim the Loon-A-See blob. |
| [38 — Connect Google Doc](38-connect-google-doc.md) | 🚧 built 2026-08-01 (branch `prd-38-google-doc`) — **blocked on an owner Google Cloud console change**: the picker needs the Drive + Picker APIs enabled, the `drive.file` scope on the consent screen, and **two new public env vars** the PRD didn't predict (`NEXT_PUBLIC_GOOGLE_PICKER_API_KEY`, `NEXT_PUBLIC_GOOGLE_CLOUD_PROJECT_NUMBER` — Google requires `setDeveloperKey` + `setAppId` for a `drive.file` picker). Until then the door renders nowhere and the band is today's three buttons. Also widened PRD 28's CSP for three Google hosts. Seam check 8/8; engine checks still 40/40 + 19/19; no live walk | most | **Thin door onto PRD 37's engine.** Fourth button on the intake band, filled/primary per Dan's spec (Google Doc, then Photo / Paste Text / Voice as outline): Google Picker with `drive.file` per-pick consent (PRD 05's identity layer, token client-side only), export the chosen Doc as plain text, hand it to the **unchanged** `extractPaste` — same caps, fencing, credential catch, review, retention. No migration, no new env; one owner console change (Drive API + scope on the existing OAuth client). Env-gated so unconfigured builds keep today's three-button band. |
| [39 — Onboarding That Grows the Tree](39-onboarding-grows-the-tree.md) | ✅ **shipped** (PR #49 merged `381d456` 2026-08-01; both migrations applied to prod; reviewer live-walked the whole recipe same day with a real test invitee: warm email rendered in Gmail → prefilled login → three-step onboarding where the inviter's "my child" answer became a one-tap parent suggestion and generation auto-suggested 4 → person + stub + 2 edges + 4 revisions verified at the DB → tree rendered the new member connected → guest negatives passed → all test rows cleaned up. Walk found 2 polish fixes, shipped as PR #51) | 35–38 | **Two slices.** A: invitations finally send an email (Resend infra exists; the invitee currently learns about the site only by word of mouth) with a three-step how-to + `/login?email=` prefill, and the inviter records "who are they to you?" on the invitation. B: the welcome flow becomes three short steps whose middle step places the member in the tree (parents + spouse pickers over existing `people`, claim-or-create own person row, stub creation, edges written on save, generation *suggested* from parent + 1 under the top-anchored scheme). Nudge persists until tree placement. Evidence: `relationships` was empty until the 2026-08-01 reset; Mike's abandoned onboarding; Dan's tree-shaped prose in "Relationship notes". |
...
| [40 — Trust Document Vault](40-trust-document-vault.md) | 🚧 **slice 1 (vault) ✅ live on prod 2026-08-30** (PR #52 merged, migration applied, managers seated Dan + Dad, negative suite passed — cleared for Dad's first real upload); **slice 2 (inferred taxonomy) ✅ live on prod 2026-08-30** (PR #53 merged, eval GATE PASS, migration applied + verified; first real Organize press awaits a non-empty register); **slice 3 (notebook intake) ✅ merged + migration live 2026-08-30** (PR #54; RLS suite passed; **the handwriting eval remains the SHIP GATE** — family photographs the 7-page corpus per `evals/trust-note/README.md` before the real notebook is read). **All PRD 40 build work complete** | most | **The trust-doc security foundation** — answers the security-model open decision that has gated 07/08. Supabase Storage private `trust` bucket, **default-deny explicit-grant access** (`trust_document_access` + `trust_managers`, not `is_admin()`), append-only audit log, agent-reads-as-the-user (no service-role key), adviser/accountant as guest-role accounts + grants. Slice 2: notebook scan OCR (Smart Intake pattern, `trust_note` intent) with per-item approve/deny into `trust_annotations`. Unblocks 07 once signed off + slice 1 ships. |
| [07 — Trust-doc RAG](07-trust-doc-rag.md) | 🔴 blocked | — | Security model now drafted in [40](40-trust-document-vault.md) — still gated on that PRD's sign-off + slice 1 shipping. |
