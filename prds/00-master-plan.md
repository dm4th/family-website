...
| [39 — Onboarding That Grows the Tree](39-onboarding-grows-the-tree.md) | 🚧 built + reviewed 2026-08-01 (branch `prd-39-onboarding`) — both slices code-complete; reviewed against a real local Postgres (functional tests as authenticated JWTs, rolled back), **two required fixes applied and re-verified**: a suggestion-accept that wiped hand-picked parents, and a claim race the partial unique index doesn't catch. `tsc`/`eslint`/`build` green + 13/13 generation checks. **Migrations validated locally, not yet applied to prod; no live walk yet.** Adds `place_self_in_tree()` (SECURITY INVOKER, atomic placement) + `my_invitation_hint()` (SECURITY DEFINER, because invitations RLS hides the invitee's own row). Also fixed a pre-existing nested-`<form>` bug that made "Finish Later" submit the step instead of skipping. **Ship order: apply both migrations before merging the code** (the welcome page calls `my_invitation_hint()` on load) | 35–38 | **Two slices.** A: invitations finally send an email (Resend infra exists; the invitee currently learns about the site only by word of mouth) with a three-step how-to + `/login?email=` prefill, and the inviter records "who are they to you?" on the invitation. B: the welcome flow becomes three short steps whose middle step places the member in the tree (parents + spouse pickers over existing `people`, claim-or-create own person row, stub creation, edges written on save, generation *suggested* from parent + 1 under the top-anchored scheme). Nudge persists until tree placement. Evidence: `relationships` was empty until the 2026-08-01 reset; Mike's abandoned onboarding; Dan's tree-shaped prose in "Relationship notes". |
