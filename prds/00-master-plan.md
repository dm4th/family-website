...
| [11 — Family Legacy](11-family-legacy.md) | 🚧 in progress — **slice 1 (Photo Archive) ✅ shipped 2026-06-30**; slices 2–4 ready · **reqs locked 2026-06-30** | — | **Lives inside the Family zone (not a new top-level zone)** — historical photo archive, family tree, timeline, stories. Build as four sequenced slices: **Photo Archive → Family Tree → Timeline → Stories** (Archive-first confirmed). **Slice 1 shipped**: `albums`/`album_photos`/`photo_people` + photo fuzzy-dating, `/family/archive` routes, nav + homepage gateway. Slice 1's keystone `people` table (ancestors who'll never log in) is live. **Requirements locked (dad's headline interest):** slice 2 tree must be **genuinely traversable** (recenter/expand/pan-zoom viz, not a static chart); slice 3 timeline must be traversable (scroll + decade jump + filter by person/branch); adding tree branches must **not** require creating accounts (handled by `people.profile_id` nullable). Consumes the [12](12-authoring-ux.md) authoring layer. Absorbs PRD 10. |
