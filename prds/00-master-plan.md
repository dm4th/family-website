...
| [11 — Family Legacy](11-family-legacy.md) | 🚧 in progress — **slices 1 (Photo Archive) & 2 (Family Tree) ✅ shipped 2026-06-30**; slices 3–4 ready · **reqs locked 2026-06-30** | — | **Lives inside the Family zone (not a new top-level zone)** — historical photo archive, family tree, timeline, stories. Build as four sequenced slices: **Photo Archive → Family Tree → Timeline → Stories** (Archive-first confirmed). **Slice 1 shipped**: `albums`/`album_photos`/`photo_people` + photo fuzzy-dating, `/family/archive` routes. **Slice 2 shipped**: `relationships` graph edges, person + edge authoring (adds ancestors with **no account**), a traversable ego-centric `/family/tree` (recenter/pan/zoom) + `/family/tree/[personId]` with in-memoriam treatment, nav + homepage gateway. _(Slice 2 migration built + PR'd; **not yet applied to prod** — awaiting go-ahead.)_ The keystone `people` table (ancestors who'll never log in) is live. **Requirements locked (dad's headline interest):** slice 3 timeline must be traversable (scroll + decade jump + filter by person/branch). Consumes the [12](12-authoring-ux.md) authoring layer. Absorbs PRD 10. |
