...
| [38 — Connect Google Doc](38-connect-google-doc.md) | 🟢 ready (2026-08-01) | most | **Thin door onto PRD 37's engine.** Fourth button on the intake band, filled/primary per Dan's spec (Google Doc, then Photo / Paste Text / Voice as outline): Google Picker with `drive.file` per-pick consent (PRD 05's identity layer, token client-side only), export the chosen Doc as plain text, hand it to the **unchanged** `extractPaste` — same caps, fencing, credential catch, review, retention. No migration, no new env; one owner console change (Drive API + scope on the existing OAuth client). Env-gated so unconfigured builds keep today's three-button band. |
