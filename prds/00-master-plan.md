...
| [29](29-older-user-readability.md) | Older-user readability & touch targets | 🔍 in review (built + live-verified 2026-07-02, [PR #30](https://github.com/dm4th/family-website/pull/30)) | all | Tailwind type-scale remap in `globals.css` (`--text-xs` 13px / `--text-sm` 15px) + `--foreground-subtle` contrast (4.65:1) + `ui/button` h-10 / `ui/input` h-10 + booking labels/instruction to 15px sentence case + chip-remove 32px + shell body copy to 16px. |
