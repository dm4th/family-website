...
**Status**: 🚧 **slices 1 + 2 ✅ live on prod** (slice 1: the vault, PR #52; slice 2: inferred taxonomy, PR #53 merged `604d5e1` 2026-08-30 — reviewed, eval GATE PASS, migration `20260831000001` applied and verified on prod). Cleared for Dad's first real Dropbox drag; the first real Organize press follows it (the on-prod Organize walk needs a non-empty register). **Slice 3 (notebook intake) ✅ merged + migration live on prod 2026-08-30** (PR #54 merged `9be0a66`; migration `20260831000002` applied and verified; reviewer's 10-check RLS suite passed). **All build work on this PRD is done.** Remaining, all human-paced: (1) the handwriting eval — THE SHIP GATE — needs the family-photographed 7-page corpus (`evals/trust-note/README.md`, ideally two pages in Dad's hand) and a GATE PASS before the flow reads the real notebook; (2) Dad's Dropbox drag + first Organize press; (3) the on-prod notebook walk with one real non-sensitive page. See Implementation below.
...
## Implementation (slice 3 — notebook intake, built 2026-08-30)

**Status**: built + validated (`tsc`/`eslint`/`build` green; all four `/advisory` routes build); **migration NOT applied; the handwriting eval is authored but NOT run and is the SHIP GATE** — the flow must not be trusted with the real notebook until it passes. The eval corpus requires photographed handwriting only the family can produce (protocol in `evals/trust-note/README.md`, ideally with pages in Dad's hand).

**Key files**

- `supabase/migrations/20260831000002_trust_annotations.sql` — `trust_annotations` (pending/approved/denied, mapping columns, manager-only RLS all four verbs, active-only restrictive; CASCADE from the scan, SET NULL from the mapped document; denied rows kept so re-reads can't re-propose them); a `trust_document_pages` manager-UPDATE policy (re-reads upsert transcriptions); event kinds `scan_read` / `annotation_approved` / `annotation_denied` added to the constraint + the manager list of the hardened insert policy. **The corpus rule is written into the migration header**: PRD 07's trusted corpus is kind='document' pages + APPROVED annotations, never raw scan OCR (scan transcriptions in the pages table serve the review screen; the slice-2 taxonomy pass already filters by kind).
- `src/lib/trust/notebook.ts` — `readTrustScan()` (transcription-first vision call: verbatim, `[unclear]` over guessing, key points each resting on a `sourceQuote`, honest confidence, empty-list-is-correct; Sonnet 5 default via `TRUST_NOTE_MODEL`, 16000 `max_tokens` with the truncation branch per the slice-2 lesson) + `proposeScanMappings()` (judges fit against caller-supplied candidates only — the parser rejects any (document, page) that wasn't offered, so the model cannot introduce a document the search didn't) + `selectMappingCandidates()` (plain term-overlap ranking; embeddings stay in PRD 07). Validate-don't-trust parsers throughout.
- `/advisory/notebook` — the manager work queue (per-scan status, Read This Page / Read Again, Review links) and `review/[id]` — transcription beside the audited Open button, pending points as edit-then-approve cards (wording editable, document link + page changeable, source quote shown as receipts), Approve / Set Aside per point, judged points listed read-only below.
- `actions.ts` — `extractTrustScan` (audit-or-abort log-first `scan_read`; transcription upserted; **pending points replaced, judged points never re-litigated** — re-proposals matching an approved/denied text are dropped; mapping best-effort: a mapping failure costs links, never points), `approveTrustAnnotation` (edited text + validated mapping, log-first), `denyTrustAnnotation` (log-first, denied kept).
- `evals/trust-note/` — six invented Birchwater note texts a person handwrites and photographs (print, cursive, crossed-out words, margin notes, an ambiguous figure, one no-trust-content page), fixture document pages for mapping. **Gates: zero fabricated points (scored against ground truth so OCR hallucinations count), zero restraint violations, zero forced mappings.** Accuracy + recall reported for eyeball judgment. Harness verified end-to-end (reports INCOMPLETE per missing photo).

**Review outcomes (PR #54, 2026-08-30/31)**: reviewer's 10-check RLS suite passed in full (manager transcription updates, `created_by` pin, guest sees zero annotations, no event forgery, cascade on scan delete) and cleared the migration for prod. Required fix landed same day, notably **in the eval, not the app**: the forced-mapping gate's only test surface was two pages where mapping is defensible, so a correct model would have failed and invited weakening the real prompt — recalibrated with per-page `allowedReferences`, note-02 promoted to planted (its text names the restatement), and a new note-07 whose content matches no fixture document (the corpus is seven pages now). Should-fix landed: 20MB early size guard in `readTrustScan` (API 32MB cap x base64 inflation vs the vault's 50MB). Nits taken: normalized judged-point dedupe with the "never" claim softened, verdict flips documented as audited manager recourse, stale-page cleanup on shorter re-reads, un-decoded HEIC rejected at upload with the fix in the message. Accepted as-is: stop-word filtering waits for PRD 07's embeddings.

**Owner steps, in order**

1. `supabase db push` — applies `20260831000002_trust_annotations.sql`.
2. **Produce the eval corpus** (handwrite + photograph the six fixture texts per `evals/trust-note/README.md`; at least two pages in Dad's hand ideally) and run: `TRUST_NOTE_CORPUS=/path npx tsx --env-file=.env.local evals/trust-note/eval.mts` — must gate PASS; record the dated results file. **The notebook flow is not cleared for the real notebook before this.**
3. On-prod walk with one real (non-sensitive) notebook page: read → review → approve one point with a mapping → deny one → re-read the page and confirm judged points stay judged → audit rows for all of it → forged `scan_read`/`annotation_approved` inserts by a granted guest fail at RLS.
